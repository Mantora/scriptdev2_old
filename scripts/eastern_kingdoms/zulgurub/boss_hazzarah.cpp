    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiIllusionsTimer < uiDiff)
        {
            ...
            for(int i = 0; i < 3; ++i)
            {
                ...
                {
                    if (Creature* pIllusion = m_creature->SummonCreature(NPC_NIGHTMARE_ILLUSION, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 30000))
