...
struct MANGOS_DLL_DECL boss_ingvarAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        if (m_pInstance)
            m_pInstance->SetData(DATA_INGVAR_EVENT, NOT_STARTED);
    }
    ...
    void EnterCombat(Unit *who)
    {
        ...
        if (m_pInstance)
            m_pInstance->SetData(DATA_INGVAR_EVENT, IN_PROGRESS);
    }
    ...
    void JustDied(Unit* killer)
    {
        ...
        if (m_pInstance)
            m_pInstance->SetData(DATA_INGVAR_EVENT, DONE);
    }
...
struct MANGOS_DLL_DECL mob_annhylde_the_callerAI : public ScriptedAI
{
    ...
    void Reset()
    {
        m_creature->AddSplineFlag(SPLINEFLAG_FLYING/* | MONSTER_MOVE_HOVER*/);
    ...
    void UpdateAI(const uint32 diff)
    {
        if (Resurect_Timer)
            if (Resurect_Timer <= diff)
            {
                ...
                {
                    Creature* ingvar = Unit::GetCreature(*m_creature, m_pInstance ? m_pInstance->GetData64(DATA_INGVAR) : 0);
                    if (ingvar)
                    {
                        ...
                        CAST_AI(boss_ingvarAI, (c_ingvar->AI()))->StartZombiePhase();
