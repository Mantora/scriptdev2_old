...
struct MANGOS_DLL_DECL boss_kelesethAI : public ScriptedAI
{
    ...
    void Reset() 
    {
        ...
        if (m_pInstance)
            m_pInstance->SetData(DATA_PRINCEKELESETH_EVENT, NOT_STARTED);
    }
    ...
    void JustDied(Unit* killer)
    {
        ...
        if (m_pInstance)
            m_pInstance->SetData(DATA_PRINCEKELESETH_EVENT, DONE);
    }
    ...
    void EnterCombat(Unit* who)
    {
        ...
        if (m_pInstance)
            m_pInstance->SetData(DATA_PRINCEKELESETH_EVENT, IN_PROGRESS);
    }
...
struct MANGOS_DLL_DECL mob_vrykul_skeletonAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        if (m_pInstance && m_pInstance->GetData(DATA_PRINCEKELESETH_EVENT) == IN_PROGRESS)
        {
            if (isDead)
            {
