...
struct MANGOS_DLL_DECL boss_svalaAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if(m_uiSacrificeEndTimer < uiDiff && m_bIsSacrifice)
        {
            for(uint8 i=0; i<3; ++i)
            {
                Unit* pAdd = m_creature->GetMap()->GetUnit(ObjectGuid(m_uiAddsGUID[i]));
                if(pAdd && pAdd->isAlive())
                {
                    Unit* pPlayer = m_creature->GetMap()->GetUnit(ObjectGuid(m_uiPlayerGUID));
                    if(pPlayer)
        ...
        if(m_uiCallFlamesTimer < uiDiff)
        {
            ...
            {
                Unit *TargetedPlayer = m_creature->GetMap()->GetUnit(ObjectGuid((*itr)->getUnitGuid()));  
